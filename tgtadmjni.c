 ...
 * 02110-1301 USA
 */
#include "com_hcd_common_TgtAdminJNI.h"
...
struct option const long_options[] = {
  ...
  {"volumesize",        required_argument, NULL, 'x'},
...
static char *short_options =
  "dhVL:o:m:t:s:c:l:n:v:b:E:f:y:x:T:I:Q:u:p:H:FP:B:Y:OC:S:";
...
// customized function from ipc_mgmt_rsp, we provide received rsp data to bufin.
static int ipc_mgmt_rsp_with_buf(int fd, struct tgtadm_req *req, int bufmaxsize, char **bufout, int *buflen) {
  ...
  if (bufmaxsize < len) {
    eprintf("not enough length in allocated buf.\n");
    return -ENOMEM;
  }

  if (buflen && bufout) {
    *buflen = len;
    buf = (*bufout) = (char *) calloc(len, sizeof(char));
  } else {
    eprintf("NULL parameters, memory crashed.\n");
    return 0;
  }
...
// customized function from ipc_mgmt_req, we provide received rsp data to bufin.
static int ipc_mgmt_req_with_buf(struct tgtadm_req *req, struct concat_buf *b,
                                 int bufmaxsize, char **bufout, int *buflen) {
  int err, fd = 0, done = 0;
  ...
  err = ipc_mgmt_rsp_with_buf(fd, req, bufmaxsize, bufout, buflen);
  out:
  if (fd > 0)
    close(fd);
...
/* params:
 * argc, argv: same as command line input;
 * bufmaxsize: max buf size we support, default is 1M defined by Java code.
 *
 * bufout: pointer address for the allocated memory; 
 * buflen: address of real buf length.
 */
int calltgtadm(int argc, char *argv[], int bufmaxsize, char **bufout, int *buflen) {
  int ch, longindex, rc;
  ...
  bsoflags = blocksize = volumesize = user = password = op_name = NULL;
  force = 0;
  optind = 0;
  opterr = 0;
  while ((ch = getopt_long(argc, argv, short_options,
  ...
  if (optind < argc) {
    eprintf("unrecognized option at index %d '%s'\n", optind, argv[optind]);
    usage(1);
    return -1;
  }
  ...
  if (mode == MODE_TARGET) {
    ...
    switch (op) {
      ...
      case OP_UNBIND:
        ...
        break;
      case OP_UPDATE:
        rc = verify_mode_params(argc, argv, "LmoyxtnvC");
        if (rc) {
          eprintf("target mode: option '-%c' is not " \
            "allowed/supported\n", rc);
          return -1;
  ...
  if (mode == MODE_DEVICE) {
    ...
    switch (op) {
      case OP_NEW:
        rc = verify_mode_params(argc, argv, "LmofyxtlbEYCSP");
        if (rc) {
      ...
      case OP_STATS:
        ...
        break;
      case OP_UPDATE:
        rc = verify_mode_params(argc, argv, "LmofyxtlPC");
        if (rc) {
  ...
  if (blocksize)
    concat_printf(&b, "%sblocksize=%s", concat_delim(&b, ","),
                  blocksize);
  if (volumesize)
    concat_printf(&b, "%svolumesize=%s", concat_delim(&b, ","),
                  volumesize);
  if (targetname)
  ...
  return ipc_mgmt_req_with_buf(req, &b, bufmaxsize, bufout, buflen);
}

JNIEXPORT jint JNICALL Java_com_hcd_common_TgtAdminJNI_execute
  (JNIEnv *env, jobject object, jobjectArray argvs, jbyteArray jbuf) {

  char *argvalue[30];
  int argcount = (*env)->GetArrayLength(env, argvs);

  for (int i = 0; i < argcount; i++) {
    jstring jstr = (jstring)((*env)->GetObjectArrayElement(env, argvs, i));
    argvalue[i] = (*env)->GetStringUTFChars(env, jstr, NULL);
  }

  jsize jsize = (*env)->GetArrayLength(env, jbuf);
  char *cbuf = NULL;
  int cbuflen = 0;

  int rc = calltgtadm(argcount, argvalue, jsize, &cbuf, &cbuflen);

  if (rc != 0) {
    eprintf("error execute tgtadmjni.\n");
  } else if (cbuf != NULL && cbuflen > 0) {
    rc = cbuflen;
    (*env)->SetByteArrayRegion(env, jbuf, 0, cbuflen, cbuf);
  }

  for (int i = 0; i < argcount; i++) {
    jstring jstr = (jstring)((*env)->GetObjectArrayElement(env, argvs, i));
    (*env)->ReleaseStringUTFChars(env, jstr, argvalue[i]);
  }
